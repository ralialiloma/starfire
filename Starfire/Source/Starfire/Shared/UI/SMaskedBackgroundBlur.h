...
/**
* A MaskedBackgroundBlur is similar to a Border in that it can be used to contain other widgets.
...
*/
class SMaskedBackgroundBlur : public SCompoundWidget
{
	SLATE_DECLARE_WIDGET_API(SMaskedBackgroundBlur, SCompoundWidget, STARFIRE_API)

public:
	SLATE_BEGIN_ARGS(SMaskedBackgroundBlur)
		: _HAlign(HAlign_Fill)
		, _VAlign(VAlign_Fill)
...
public:
	STARFIRE_API void Construct(const FArguments& InArgs);
	STARFIRE_API virtual int32 OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const override;

	STARFIRE_API void SetContent(const TSharedRef<SWidget>& InContent);
	STARFIRE_API void SetApplyAlphaToBlur(bool bInApplyAlphaToBlur);
	STARFIRE_API void SetBlurRadius(TAttribute<TOptional<int32>> InBlurRadius);
	STARFIRE_API void SetBlurStrength(TAttribute<float> InStrength);
	STARFIRE_API void SetCornerRadius(TAttribute<FVector4> InCornerRadius);
	STARFIRE_API void SetLowQualityBackgroundBrush(const FSlateBrush* InBrush);
	
	STARFIRE_API void SetHAlign(EHorizontalAlignment HAlign);
	STARFIRE_API void SetVAlign(EVerticalAlignment VAlign);
	STARFIRE_API void SetPadding(TAttribute<FMargin> InPadding);

	STARFIRE_API bool IsUsingLowQualityFallbackBrush() const;

protected:
	STARFIRE_API void ComputeEffectiveKernelSize(float Strength, int32& OutKernelSize, int32& OutDownsampleAmount) const;
