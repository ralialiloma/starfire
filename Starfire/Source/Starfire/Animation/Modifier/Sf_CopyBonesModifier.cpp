...
#include "Sf_CopyBonesModifier.h"
...
USf_CopyBonesModifier::USf_CopyBonesModifier()
	:Super()
{
}

void USf_CopyBonesModifier::OnApply_Implementation(UAnimSequence* Animation)
{
	...
	for (const FSf_BoneReferencePair& Pair : BonePairs)
	{
	...
	for (int32 AnimKey = 0; AnimKey < NumKeys; AnimKey++)
	{
		for (FCopyBoneData& Data : CopyBoneDataContainer)
		{
			...
			if (!Model->IsValidBoneTrackName(Data.TargetBoneName))
			{
				Controller.AddBoneCurve(Data.TargetBoneName);
			}
...
void USf_CopyBonesModifier::OnRevert_Implementation(UAnimSequence* Animation)
{
